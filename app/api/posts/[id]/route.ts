...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    // Update the post (in production, this would update the database)
    ...
    return NextResponse.json({ post: SAMPLE_POSTS[postIndex] })
  } catch (error) {
...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    // Remove the post (in production, this would delete from database)
    SAMPLE_POSTS.splice(postIndex, 1)
