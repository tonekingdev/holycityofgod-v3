...
export function AnnouncementsList({
  ...
  const fetchAnnouncements = useCallback(async () => {
    try {
      ...
      console.error("[Anointed Innovations] Error fetching announcements:", err)
    ...
    <div className={`space-y-6 ${className || ""}`}>
      ...
      {(filters.type || filters.priority || searchQuery) && (
        <div className="flex flex-wrap gap-2">
          ...
          {filters.type && (
            <Badge variant="secondary">Type: {ANNOUNCEMENT_TYPES.find((t) => t.value === filters.type)?.label}</Badge>
          )}
          {filters.priority && (
